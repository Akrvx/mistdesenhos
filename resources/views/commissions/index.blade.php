                        ...
                        @foreach($myRequests as $req)
                            <div class="glass-card p-5 rounded-2xl border border-white/5 relative overflow-hidden group">
                                ...
                                @if($req->prazo_desejado)
                                    <p class="text-xs text-slate-500">Prazo estipulado: {{ \Carbon\Carbon::parse($req->prazo_desejado)->format('d/m/Y') }}</p>
                                @endif
                                ...
                                @elseif($req->status == 'active')
                                    <div class="mt-4 pt-4 border-t border-white/10 text-center">
                                        <span class="text-purple-300 text-xs font-bold">
                                            🎨 Artista está trabalhando na sua arte.
                                        </span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="glass-card p-5 rounded-2xl border border-white/5 relative">
                                ...
                                @if($order->status == 'pending')
                                    <form action="{{ route('commissions.update-status', $order->id) }}" method="POST" class="space-y-3">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            ...
                                            <div>
                                                <label class="text-[10px] uppercase font-bold text-slate-500">Prazo de Entrega</label>
                                ...
                                @elseif($order->status == 'accepted')
                                    <div class="text-center py-3 bg-yellow-500/10 rounded-lg border border-yellow-500/20">
                                        ...
                                        <p class="text-slate-400 text-xs mt-1">Combinado: 🪙 {{ $order->price }} para dia {{ \Carbon\Carbon::parse($order->prazo_desejado)->format('d/m') }}</p>
                                    </div>

                                @elseif($order->status == 'active')
                                    <div class="text-center py-3 bg-purple-500/10 rounded-lg border border-purple-500/20 mb-3">
                                        <p class="text-purple-400 text-xs font-bold">💰 Pago! Mãos à obra.</p>
                                        <p class="text-slate-400 text-xs">Entregar até: {{ \Carbon\Carbon::parse($order->prazo_desejado)->format('d/m/Y') }}</p>
                                    </div>
