...
@section('content')
    ...
    <div class="w-full min-h-screen pt-32 pb-12">
        ...
        <div class="text-center py-20 px-4 animate-fade-in-up relative z-10">
            ...
            @guest
                <div class="flex justify-center gap-6 mt-8">
                    <a href="{{ route('register') }}" class="px-10 py-4 bg-cyan-500 hover:bg-cyan-400 text-white rounded-full font-bold text-lg shadow-[0_0_20px_rgba(6,182,212,0.6)] transition transform hover:scale-105 hover:-translate-y-1">
                        Criar Conta
                    </a>
                    <a href="{{ route('login') }}" class="px-10 py-4 bg-white/5 hover:bg-white/10 text-white rounded-full font-bold text-lg border border-white/20 transition backdrop-blur-sm">
                        Entrar
                    </a>
                </div>
            @endguest
        ...
        <div class="max-w-[95%] mx-auto px-4 mt-8">
            ...
            @if($arts->isEmpty())
                <div class="text-center py-20 glass-card rounded-3xl border border-white/10 mx-auto max-w-2xl">
                    <p class="text-white/40 text-xl">O lago está calmo... Nenhuma arte encontrada.</p>
                </div>
            @else
                    ...
                    @foreach($arts as $art)
                        <div class="glass-card overflow-hidden rounded-3xl border border-white/10 hover:border-cyan-500/50 transition duration-500 group relative flex flex-col">
                            ...
                            <div class="aspect-[4/3] bg-black/20 relative overflow-hidden">
                                ...
                                <div class="absolute top-3 right-3 bg-black/60 backdrop-blur-md text-white font-bold px-3 py-1 rounded-full text-sm border border-white/10 shadow-lg">
                                    R$ {{ number_format($art->preco, 2, ',', '.') }}
                                </div>

                                @if($art->is_nsfw)
                                    <div class="absolute top-3 left-3 bg-red-600/90 text-white font-bold px-2 py-1 rounded-lg text-xs border border-red-400 shadow-lg">
                                        NSFW
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="p-5 flex flex-col flex-grow">
                                <div class="flex items-center gap-3 mb-3">
                                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center text-xs font-bold text-white shadow-inner">
                                    ...
                                    <div class="overflow-hidden">
                                        ...
                                        <a href="{{ route('artist.show', $art->user->id) }}" class="text-xs text-cyan-200 hover:text-white transition truncate block">
                                            por {{ $art->user->name }}
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div class="mt-auto pt-4">
                                    <a href="{{ route('arts.show', $art->id) }}" class="block w-full text-center py-2 rounded-xl bg-white/5 hover:bg-white/20 text-white text-sm font-semibold transition border border-white/10 group-hover:border-cyan-500/30">
                                        Ver Detalhes
                                    </a>
                                </div>
                            </div>
