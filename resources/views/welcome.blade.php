...
@section('content')
    ...
    <a href="{{ route('arts.show', $art->id) }}">
        @if(Str::startsWith($art->imagem_caminho, 'fake_demo'))
            <img src="https://picsum.photos/seed/{{ $art->id }}/800/800" 
                 class="w-full h-full object-cover transition duration-700 group-hover:scale-110 
                 {{ $art->is_nsfw ? 'blur-xl brightness-50 grayscale scale-110' : '' }}">
        @else
            <img src="{{ asset('storage/' . $art->imagem_caminho) }}" 
                 class="w-full h-full object-cover transition duration-700 group-hover:scale-110 
                 {{ $art->is_nsfw ? 'blur-xl brightness-50 grayscale scale-110' : '' }}">
        @endif
    </a>

    <div class="absolute top-3 right-3 bg-black/60 backdrop-blur-md text-white font-bold px-3 py-1 rounded-full text-sm border border-white/10 shadow-lg flex items-center gap-1 z-10">
    ...
    @if($art->is_nsfw)
        <div class="absolute inset-0 flex items-center justify-center z-0 pointer-events-none">
            <div class="bg-red-600/90 text-white font-bold px-4 py-2 rounded-xl text-lg border border-red-400 shadow-lg backdrop-blur-sm">
                🔞 NSFW
            </div>
        </div>
    @endif
